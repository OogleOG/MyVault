...
public class PasswordVaultFX {
    ...
    private final Preferences prefs = Preferences.userNodeForPackage(PasswordVaultFX.class);
    ...
    public static class Entry {
        ...
        long createdAt;
        long updatedAt;
        String category; // optional category/folder
    ...
    public static void main(String[] args) {
        // Theme bootstrap
        boolean dark = Preferences.userNodeForPackage(PasswordVaultFX.class)
...
// add to the Help menu
        ...
        categoryList.setFixedCellHeight(22);
...
// Theme-aware subtle color
    ...
    private void onSetCategory() {
        ...
        for (Entry e : data.entries) if (e.category != null && !e.category.isBlank()) cats.add(e.category.trim());
        JComboBox<String> box = new JComboBox<>(cats.toArray(new String[0]));
        box.setEditable(true);
        if (sel.category != null) box.setSelectedItem(sel.category);
        int r = JOptionPane.showConfirmDialog(frame, box, "Set Category", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        if (r == JOptionPane.OK_OPTION) {
            Object v = box.getEditor().getItem();
            String cat = (v == null) ? null : v.toString().trim();
...
private JPopupMenu createRowMenu() {
        ...
        JMenuItem setCat = new JMenuItem("Set Category…"); setCat.addActionListener(e -> onSetCategory());
        m.add(edit); m.addSeparator(); m.add(copyU); m.add(copyE); m.add(copyP); m.addSeparator(); m.add(setCat); m.addSeparator(); m.add(del);
        return m;
    }
    ...
    private Entry findById(String id) {
        ...
        for (Entry e : data.entries) if (id.equals(e.id)) return e;
        return null;
    }
    ...
    private void applyFilters() {
        ...
        // Category filter
        final String sel = (categoryList != null) ? categoryList.getSelectedValue() : null;
        if (sel != null && !"All".equals(sel)) {
            RowFilter<TableModel,Object> catFilter = new RowFilter<TableModel,Object>() {
                @Override
                public boolean include(javax.swing.RowFilter.Entry<? extends TableModel, ? extends Object> row) {
                    // Adjust column index if your ID column is different:
                    Object idObj = row.getValue(4); // ID column (Label,Username,Email,Updated,ID)
                    String id = (idObj == null) ? null : idObj.toString();

                    // Explicitly refer to your model class to avoid clash
                    PasswordVaultFX.Entry en = findById(id);
                    String cat = (en == null) ? null : en.category;

                    if ("Uncategorized".equals(sel)) {
                        return cat == null || cat.isBlank();
                    } else {
                        return sel.equalsIgnoreCase(cat == null ? "" : cat.trim());
                    }
                }
            };
